...
export class MembersService {
  ...
  async get(choirId: string, membershipId: string) {
  ...
  async create(choirId: string, dto: CreateMemberDto) {
    ...
    if (
      ...
      !(await this.prisma.voiceSection.findFirst({
        where: { id: dto.voiceSectionId, choirId },
      }))
    )
      throw new NotFoundException('Voice section not found');
    const identities = [
    ...
    if (
      ...
      (await this.prisma.membership.findUnique({
        where: { userId_choirId: { userId: existing.id, choirId } },
      }))
    )
      throw new ConflictException('Member already belongs to this choir');
    ...
    return this.prisma.$transaction(async (tx) => {
        ...
        data: {
          ...
          profile: {
            create: {
              ...
              functionTitle: dto.functionTitle,
              voiceSectionId: dto.voiceSectionId,
              joinedChoirAt: new Date(),
            },
          },
        },
      });
      const role = await tx.role.findFirst({
        where: { organizationId: choir.organizationId, code: 'MEMBER' },
      });
      if (role)
      ...
      if (dto.voiceSectionId)
        await tx.voiceSectionAssignment.create({
          data: {
  ...
  async update(choirId: string, membershipId: string, dto: UpdateMemberDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      if (voiceSectionId) {
        await tx.voiceSectionAssignment.updateMany({
          where: { membershipId, endsAt: null },
          data: { endsAt: new Date() },
        });
        await tx.voiceSectionAssignment.create({
          data: { membershipId, voiceSectionId },
        });
      }
      if (status)
      ...
      const consent =
        whatsappConsent === undefined
          ...
          : {
              ...
              whatsappConsentSource: whatsappConsent
                ? whatsappConsentSource || 'admin'
                : null,
            };
  ...
  async archive(choirId: string, membershipId: string) {
    await this.get(choirId, membershipId);
